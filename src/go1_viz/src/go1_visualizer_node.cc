...
// 引入你的 Go1 IK 头文件
#include "go1_inverse_kinematics.h"

using namespace xpp;

int main(int argc, char *argv[])
{
  ::ros::init(argc, argv, "go1_visualizer_node");
  ...
  // 实例化你的 Go1 IK 求解器
  auto go1_ik = std::make_shared<Go1InverseKinematics>();
  ...
  // 定义参数名称和坐标系
  std::string urdf_param = "go1_rviz_urdf_robot_description";
  std::string base_link = "base"; // go1.urdf 里基座叫 "base"
  std::string world_frame = "world";
  std::string tf_prefix = "go1_des"; // 给 TF 加个前缀，防止冲突

  // 创建可视化器
  // 它会订阅 joint_desired_go1 并在 Rviz 里画图
  UrdfVisualizer go1_desired(
      urdf_param, 
      joint_names, 
      base_link, 
      world_frame,
